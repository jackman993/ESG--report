"""Lightweight PPT engine supporting Layout A (text-right image) and Layout B (left media, right text)."""
...
from config_pptx import PPT_CONFIG, SLIDE_CONFIGS, SEED_TEMPLATE_PATH, OUTPUT_PATH
...
class PPTFullEngine:
        ...
        if len(self.prs.slides) > 0:
            ...
            for idx, slide in enumerate(self.prs.slides):
                ...
                try:
                    ...
                    # 如果這個頁面完全沒有 placeholder，使用它的母版
                    if placeholder_count == 0:
                        self.blank_layout = slide.slide_layout
                        layout_name = self.blank_layout.name if hasattr(self.blank_layout, 'name') else 'Unknown'
                        print(f"[INFO] 找到乾淨頁面（第 {idx + 1} 頁），使用其母版: {layout_name}")
                        break
                except Exception as e:
                    print(f"[WARN] 檢查第 {idx + 1} 頁時出錯: {e}")
                    continue
            
            # 如果沒找到完全乾淨的，使用第一個頁面的母版（fallback）
            if self.blank_layout is None:
                self.blank_layout = self.prs.slides[0].slide_layout
                layout_name = self.blank_layout.name if hasattr(self.blank_layout, 'name') else 'Unknown'
                print(f"[WARN] 未找到完全乾淨的頁面，使用第一頁的母版: {layout_name}")
        else:
            # 如果模板沒有現有頁面，使用最後一個 layout（通常是最簡單/最空白的）
            if len(layouts) > 0:
                self.blank_layout = layouts[len(layouts) - 1]
                layout_name = self.blank_layout.name if hasattr(self.blank_layout, 'name') else f'Layout {len(layouts)-1}'
                print(f"[INFO] 使用最後一個母版: {layout_name}")
            else:
                self.blank_layout = layouts[0]
                print(f"[WARN] 使用第一個母版（fallback）")
        
        # 預先建立所有頁面（避免在生成過程中 add/clear，導致 PowerPoint 結構問題）
        self.total_slides = PPT_CONFIG.get("total_slides", 18)
        
        # 先創建 5 頁空白頁來確保母版是乾淨的
        # 然後用這個乾淨的母版來生成所有頁面
        existing_slides = len(self.prs.slides)
        needed_slides = self.total_slides
        
        # 確保總頁數正好是18頁（如果模板頁數不足，補足；如果過多，只使用前18頁）
        self._slides_by_index = {}
        
        # 先創建 5 頁空白頁（確保母版是乾淨的）
        for i in range(5):
            new_slide = self.prs.slides.add_slide(self.blank_layout)
            self._clear_placeholder_text_only(new_slide)
        
        # 現在用這個乾淨的母版補足到 18 頁
        while len(self.prs.slides) < needed_slides:
            new_slide = self.prs.slides.add_slide(self.blank_layout)
            # 立即清除所有 placeholder 中的預設文字（如「按一下已新增文字」）
            self._clear_placeholder_text_only(new_slide)
            idx = len(self.prs.slides)
            self._slides_by_index[idx] = new_slide
        
        # 確保索引字典包含所有18頁的引用，並清除所有現有頁面的 placeholder 文字
        for idx in range(1, needed_slides + 1):
            if idx not in self._slides_by_index:
                slide = self.prs.slides[idx - 1]
                self._clear_placeholder_text_only(slide)
                self._slides_by_index[idx] = slide
        
        print(f"[INFO] 預先建立 {self.total_slides} 張投影片完成（模板原有 {existing_slides} 頁，總共 {len(self.prs.slides)} 頁，索引字典 {len(self._slides_by_index)} 項）")
    ...
    def generate(self):
        ...
        # 基礎檔名
        base_name = PPT_CONFIG.get("output_filename", "ESG_PPT_AB.pptx")
        base = Path(base_name)
    ...
    def generate_subset(self, slide_indices: List[int], output_filename: Optional[str] = None):
        for idx in slide_indices:
            self._generate_slide(idx)
        filename = output_filename or PPT_CONFIG.get("output_filename", "ESG_PPT_AB.pptx")
    ...
    def _generate_slide(self, slide_index: int):
        ...
        if layout == "A":
            ...
            self._layout_cover(slide, cfg)
        else:
            raise ValueError(f"Unsupported layout: {layout}")
        self._add_watermark(slide, PPT_CONFIG.get("watermark_text"))

    def _ensure_slide(self, slide_index: int):
        """直接返回預先建立的 slide，不再 add/clear"""
        if slide_index in self._slides_by_index:
            return self._slides_by_index[slide_index]
        # 如果索引超出範圍（理論上不應該發生），fallback 到舊邏輯
        while len(self.prs.slides) <= slide_index - 1:
            layouts = self.prs.slide_layouts
            blank_layout = layouts[0]
        ...
        return self.prs.slides[slide_index - 1]
    ...
    # ------------------------------------------------------------------
    # Layout A (left text, right single image)
    # ------------------------------------------------------------------
    def _layout_a(self, slide, cfg):
        ...
        if background_cfg:
            ...
            if path and Path(path).exists():
                ...
                # 暫時禁用直接操作 _spTree，避免破壞 PowerPoint 結構
        ...
        if component_cfg:
            ...
            image_width = cfg.get("image_width_cm", 14.0)
            ...
            pic = slide.shapes.add_picture(
                ...
                Inches(cm(left_cm_val)),
                Inches(cm(top_cm)),
                width=Inches(cm(image_width)),
            )
            max_height_cm = cfg.get("image_max_height_cm")
            if max_height_cm:
                max_height = Inches(cm(max_height_cm))
                if pic.height > max_height:
                    scale = max_height / pic.height
                    pic.height = max_height
                    pic.width = int(pic.width * scale)
            if cfg.get("image_align") == "right":
                column_right = Inches(cm(left_cm_val)) + Inches(cm(image_width))
                pic.left = column_right - pic.width
    ...
    # ------------------------------------------------------------------
    # Layout B (left media, right text)
    # ------------------------------------------------------------------
    def _layout_b(self, slide, cfg):
        ...
        if mode == "stack":
            ...
            gallery_items = []
            total_height = 0
            if layout_variant == "left_stack_right_single" and len(paths) >= 2:
                left_paths = paths[:-1]
                right_path = paths[-1]
                left_items = []
                left_widths = []
            ...
            for idx, path in enumerate(paths):
                if not Path(path).exists():
                    continue
                current_width_cm = widths_override[idx] if idx < len(widths_override) else width_cm_val
                pic = slide.shapes.add_picture(
                    str(path),
                    Inches(cm(area_left)),
                    Inches(cm(area_top)),
                    width=Inches(cm(current_width_cm)),
                )
                caption_box = None
                caption_height = 0
                if idx < len(captions) and captions[idx]:
                    ...
                    if caption_box:
                        caption_box.fill.background()
                element_height = pic.height + caption_height
                gallery_items.append((pic, caption_box, element_height))
                total_height += element_height
                if idx < len(paths) - 1:
                    total_height += Inches(cm(gap_cm))
            ...
            cursor = start_top
            for idx, (pic, caption_box, element_height) in enumerate(gallery_items):
                pic.top = int(cursor)
                cursor += pic.height
                if caption_box:
    ...
    # ------------------------------------------------------------------
    # Layout C (top text, middle component/image, bottom text)
    # ------------------------------------------------------------------
    def _layout_c(self, slide, cfg):
        # 優先使用硬編碼的文本（如果存在），否則使用生成的中文文本
        top_text = cfg.get("top_text")
        bottom_text = cfg.get("bottom_text")
        
        # 如果沒有硬編碼的文本，從 content_method 生成並分割
        if not top_text and not bottom_text:
            generated_text = self._get_slide_text(cfg)
            if generated_text:
                # 將生成的中文文本分成兩部分（大約各一半）
                text_length = len(generated_text)
                split_point = text_length // 2
                # 嘗試在句號、逗號或換行符附近分割，避免切斷句子
                for delimiter in ['。', '，', '\n', '.', ',']:
                    pos = generated_text.find(delimiter, split_point - text_length // 4, split_point + text_length // 4)
                    if pos > 0:
                        split_point = pos + 1
                        break
                top_text = generated_text[:split_point].strip()
                bottom_text = generated_text[split_point:].strip()
        elif not top_text:
            # 只有 bottom_text，top_text 使用生成文本的前半部分
            generated_text = self._get_slide_text(cfg)
            if generated_text:
                split_point = len(generated_text) // 2
                for delimiter in ['。', '，', '\n', '.', ',']:
                    pos = generated_text.find(delimiter, split_point - len(generated_text) // 4, split_point + len(generated_text) // 4)
                    if pos > 0:
                        split_point = pos + 1
                        break
                top_text = generated_text[:split_point].strip()
        elif not bottom_text:
            # 只有 top_text，bottom_text 使用生成文本的後半部分
            generated_text = self._get_slide_text(cfg)
            if generated_text:
                split_point = len(generated_text) // 2
                for delimiter in ['。', '，', '\n', '.', ',']:
        ...
        if media_cfg:  # 只有當 center_media 存在時才處理
            ...
            # 如果沒有 center_media，使用預設的 top_offset_cm
            top_offset_cm = 8.5

        if bottom_text:
            # 如果沒有 center_media，使用預設的 bottom_text_top_cm
            bottom_text_top = media_cfg.get("bottom_text_top_cm", 15.5) if media_cfg else 15.5
            bottom_box = self._get_textbox(
                slide,
                left_cm=cfg.get('bottom_box_left_cm', 2.5),
                top_cm=bottom_text_top,
    ...
    # ------------------------------------------------------------------
    # Layout Cover (chapter cover page)
    # ------------------------------------------------------------------
    def _layout_cover(self, slide, cfg):
        """處理章節封面頁面：背景圖片 + 左側文字"""
        # 添加背景圖片
        image_path = cfg.get("image_path")
        if image_path and Path(image_path).exists():
            width_cm = cfg.get("image_width_cm", 29.7)
            height_cm = cfg.get("image_height_cm", 16.0)
            left_cm = cfg.get("image_left_cm", 0)
            top_cm = cfg.get("image_top_cm", 0)
        ...
        # 添加封面文字
        cover_text_cfg = cfg.get("cover_text")
        if cover_text_cfg:
            text_left_cm = cover_text_cfg.get("left_cm", 3.0)
            text_top_cm = cover_text_cfg.get("top_cm", 6.0)
            font_color = cover_text_cfg.get("font_color", (255, 255, 255))
            lines = cover_text_cfg.get("lines", [])
            
            # 計算文字框高度（根據行數）
            line_height_cm = 1.5  # 每行大約高度
            total_height_cm = len(lines) * line_height_cm + 1.0  # 加上間距
            
            text_box = self._get_textbox(
                slide,
                left_cm=text_left_cm,
                top_cm=text_top_cm,
                width_cm=25.0,
            ...
            for idx, line_cfg in enumerate(lines):
                ...
                p.text = line_cfg.get("text", "")
                p.font.name = self.font_family
                p.font.size = Pt(line_cfg.get("font_size", 32))
                p.font.bold = line_cfg.get("bold", False)
                p.font.color.rgb = RGBColor(*font_color)
                p.space_before = Pt(0)
                p.space_after = Pt(0)
                if idx < len(lines) - 1:
                    p.space_after = Pt(8)  # 行間距
    ...
    def _clear_placeholder_text_only(self, slide):
        """
        清除 slide 上所有 placeholder 中的預設文字（如「按一下已新增文字」）
        只清除文字內容，不刪除 shape，避免破壞 PowerPoint 結構
        這是一個安全的方法，不會導致「需要修復」問題
        """
        for shape in slide.shapes:
            if shape.shape_type == MSO_SHAPE_TYPE.PLACEHOLDER:
                try:
                    ...
                    if hasattr(shape, "text_frame") and shape.text_frame:
                        ...
                        for paragraph in shape.text_frame.paragraphs:
                            paragraph.text = ""
                        # 如果 text_frame 有文字，也清除
                        if shape.text_frame.text:
                            shape.text_frame.text = ""
                except Exception:
                    # 如果清除失敗，跳過（不影響其他 shape）
                    pass

    def _cleanup_slide(self, slide):
        """
        清理 slide 上非 placeholder 的 shape（改用更安全的方式，避免直接操作 _spTree）
        
        ⚠️ 此方法已被禁用，因為會破壞 PowerPoint 結構導致「需要修復」問題
        如需啟用，請取消 _generate_slide 中第111行的註釋
        """
        # 方法已被禁用，直接返回
        return
        removable = []
        ...
        # 改用 python-pptx 的標準 API 刪除 shape，避免直接操作 _spTree
        for shape in removable:
            try:
                element = shape._element
                element.getparent().remove(element)
            except Exception:
                # 如果標準 API 失敗，fallback 到舊方法（但這應該很少發生）
                try:
    ...
    def _append_notes(self, textbox, notes: Optional[List[Any]]):
        # 暫時禁用紅色文字註釋功能，避免 PowerPoint 結構問題
        return
        # if not notes:
        #     return
        # frame = textbox.text_frame
    ...
    def _apply_alert(self, shape, text: str):
        # 暫時禁用紅色邊框和標籤功能，避免 PowerPoint 結構問題
        return
        # if not text:
        #     return
        # try:
