...
TABLE_TITLE = "TCFD 轉型風險分析"
TYPE_NAME = "轉型風險"
RISK_TYPES = ['政策與法規', '綠色產品與科技']


def create_table(csv_lines, industry="企業", filename=None, output_dir=None):
    """從 CSV 生成 TCFD PPTX
    output_dir: 輸出資料夾（可選，預設使用內部 OUTPUT_DIR）
    """
    ...
    # 表格尺寸與置中計算（A4 橫向）
    slide_w = 11.69
    table_w = 10.0  # 表格寬度 ≈ 25cm
    left_margin = (slide_w - table_w) / 2  # 置中
    
    # 標題
    title = slide.shapes.add_textbox(Inches(left_margin), Inches(0.2), Inches(table_w), Inches(0.5))
    ...
    tbl = slide.shapes.add_table(rows, 6, Inches(left_margin), Inches(0.85), Inches(table_w), Inches(6.2)).table
    
    # 欄寬 (總寬 10.0" ≈ 25cm)
    tbl.columns[0].width = Inches(1.1)   # 風險類型
    tbl.columns[1].width = Inches(1.4)   # 子類型
    tbl.columns[2].width = Inches(0.9)   # 時間
    tbl.columns[3].width = Inches(1.5)   # 風險描述
    tbl.columns[4].width = Inches(2.5)   # 財務影響
    tbl.columns[5].width = Inches(2.6)   # 因應措施
    ...
    if filename is None:
        ...
        filename = f"TCFD_01_轉型風險_{industry}_{timestamp}.pptx"
